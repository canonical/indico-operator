...
RUN apt update \
    ...
    && apt update \
    && apt install -y cron gettext git libpq-dev libxmlsec1-dev locales pkg-config postgresql-client python3.9 python3.9-dev python3.9-distutils python3-pip texlive-xetex \
    && python3.9 -m pip install --prefer-binary indico~=3.1.1 indico-plugin-piwik python3-saml uwsgi \

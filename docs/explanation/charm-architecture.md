...
6. A [Celery exporter](https://github.com/danihodovic/celery-exporter) container that can be used to collect metrics from Celery.
...
The `Celery Prometheus Exporter` listens on port `9808` and metrics about Indico Celery tasks can be scraped by Prometheus there.

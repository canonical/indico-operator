...
def _format_dt(dt):
    return (
        utc_to_server(dt.replace(second=0, microsecond=0)).replace(tzinfo=None).isoformat(" ")
        if dt
        else None
    )
...
@click.option("--include-deleted", "-D", is_flag=True, help="Include deleted users in the results")
@click.option("--include-pending", "-P", is_flag=True, help="Include pending users in the results")
@click.option("--include-blocked", "-B", is_flag=True, help="Include blocked users in the results")
@click.option(
    "--include-external",
    "-X",
    is_flag=True,
...
@click.option("--include-system", "-S", is_flag=True, help="Also include the system user")
...
def search(
    ...
    **criteria,
    ...
    users = sorted(
        (u for u in res if isinstance(u, User)),
        key=lambda x: (x.first_name.lower(), x.last_name.lower(), x.email),
    )
    externals = sorted(
        (ii for ii in res if isinstance(ii, IdentityInfo)),
        key=lambda x: (
            _safe_lower(x.data.get("first_name")),
            _safe_lower(x.data.get("last_name")),
    ...
    if users:
        ...
        for user in users:
            table_data.append(
                [str(user.id), user.first_name, user.last_name, user.email, user.affiliation]
            )
...
def create(grant_admin):
    ...
    while True:
        ...
        if not User.query.filter(
            User.all_emails == email, ~User.is_deleted, ~User.is_pending
        ).has_rows():
            break
    ...
    if click.confirm(click.style(f"Create the new {user_type}?", fg="yellow"), default=True):
        ...
        db.session.commit()
        print(
            cformat("%{green}New {} created successfully with ID: %{green!}{}").format(
                user_type, user.id
            )
        )
...
@click.option(
    ...
    "-v",
    is_flag=True,
    help="Show more verbose information and include revoked tokens",
)
def token_list(user_id, verbose):
    ...
    if user is None:
        ...
        return
    _print_user_info(user)
    query = user.query_personal_tokens(include_revoked=verbose).order_by(
        PersonalToken.revoked_dt.isnot(None), db.func.lower(PersonalToken.name)
    )
...
@click.option(
    "--list-scopes",
    is_flag=True,
    callback=_show_available_scopes,
    expose_value=False,
    is_eager=True,
...
def token_create(user_id, token_name, scopes):
    ...
    token = (
        ...
        .filter(db.func.lower(PersonalToken.name) == token_name.lower())
        .first()
    )
    if token:
